...
export class LivroService {
  ...
  async cadastrar(titulo: string, autorId: number, quantidade: number = 1, anoPublicacao?: number): Promise<ILivro> {
    try {
      ...
      if (quantidade < 0) {
        throw new Error('A quantidade disponível em estoque não pode ser negativa.');
      }

      const autorExiste = await this.autorRepository.buscarPorId(autorId);
      if (!autorExiste) {
        throw new Error(`Não é possível cadastrar: O Autor com ID ${autorId} não existe no banco de dados. Cadastre o autor primeiro!`);
      }
  ...
  async atualizar(id: number, dados: Partial<Livro>): Promise<ILivro> {
    try {
      ...
      if (dados.autor_id !== undefined && !isNaN(dados.autor_id)) {
        ...
        if (!autorExiste) {
          throw new Error(`Não é possível atualizar: O Autor com ID ${dados.autor_id} não existe.`);
        }
      }
